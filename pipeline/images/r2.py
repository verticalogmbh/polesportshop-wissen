...
def build_originals_index(client, prefix: str, name_map: dict | None = None,
    ...
    # Neueste zuerst (nach jüngstem Upload je Artikel).
    ordered = sorted(groups.items(),
                     key=lambda kv: _latest(kv[1]).timestamp() if _latest(kv[1]) else 0,
                     reverse=True)
    cards = []
    for n, (artnr, items) in enumerate(ordered, start=1):
        ...
        thumbs = "".join(
            ...
            for i, u in enumerate(_urls(items)))
        cards.append(f'<section><h2>{head} <span class="badge">{marker} · #{n}</span></h2>'
...
h2{{font-size:15px;color:#444;margin:8px 0}}.badge{{color:#a0508c;font-size:12px;font-weight:600;margin-left:6px}}.g{{display:flex;flex-wrap:wrap;gap:10px}}
...
def build_master_index(client, titel: str = "Originalbilder — alle Kollektionen") -> str:
    ...
    for p in prefixes:
        ...
        entries.append((p, len(imgs), sum(len(v) for v in imgs.values()), latest, preview))
    # Neueste Kollektion zuerst, alte unten (nach jüngstem Upload in der Kollektion).
    entries.sort(key=lambda e: e[3].timestamp() if e[3] else 0, reverse=True)
    cards = []
    for n, (p, n_art, n_img, latest, preview) in enumerate(entries, start=1):
        ...
        cards.append(
            ...
            f'<div class="meta"><span class="badge">{marker} · #{n}</span>'
...
.meta .badge{{display:block;color:#a0508c;font-size:11px;font-weight:700;letter-spacing:.04em;margin-bottom:2px}}</style></head><body>
